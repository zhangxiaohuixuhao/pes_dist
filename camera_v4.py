    ...
    if len(detections) > 0:
        for detection in detections:
            if detection[0] == 'person':
                ...
                x = x * img_h / network_width(netMain)
                y = y * img_w / network_width(netMain)
                w = w * img_h / network_width(netMain)
                h = h * img_w / network_width(netMain)
                x1 = int(x)
                y1 = int(y - h / 2)
                x2 = int(x)
                y2 = int(y + h / 2)
...
class CarCamera(object):
    def __init__(self, cameraid, rount, alarmpost, deviceget):
        ...
        self.frame_queue = RQueue(max_len=25, name='Frame Queue.')
    ...
    def feed_frames(self):
        ...
        :return:
        """
        global unnormal_send, num, img_send, dist_time, interval, camera_sta, dist, put_img
        while True:
            start = time.time()
            frame = self.camera_queue.get()
            r = YOLO(frame)
            end_lab = change_lab(frame, r)
